    ...
    else // memory already been allocated, must find space or create it
    {
        ...
        while (curr) {
            ...
            { // found suitable chunk
                ...
                int size_diff = curr->size - correct_size;
                ...
                { // we have too much room, need to split.
                    ...
                    new_chunk = (chunk *)((BYTE *)curr + correct_size); // this makes sure it gets incremented by the proper amount
                    ...
                    new_chunk->next = curr->next; // insert the new chunk into linked list of chunks
                    new_chunk->prev = curr; 
                    if (curr->next) { // if current if current chunk has a chunk after it
                        ((chunk *)curr->next)->prev = new_chunk; // insert the new chunk in between
                    }
                    curr->next = new_chunk;
                    curr->size = correct_size;
                }
